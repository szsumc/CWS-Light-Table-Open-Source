...
export const STANDALONE_APP_SHELL = `<div class="app-shell">
      ...
      <div id="iconToolbar" class="icon-toolbar" role="toolbar" aria-label="Quick actions"></div>
...
export const STANDALONE_APP_SCRIPT = `function standaloneBootstrap() {
  ...
  const DEFAULT_GENERATOR_VERSION = "0.1.3";
  ...
  function bootstrapFromEmbeddedWorkbook() {
    ...
    if (!workbook) return;
    const currentFileName = inferCurrentHtmlFileName();
    state.workbook = workbook;
    state.sourceFileName = sanitizeHtmlFileName(currentFileName || workbook.sourceName || state.sourceFileName || inferSourceFileName());
  ...
  function inferCurrentHtmlFileName() {
    const path = String(window.location.pathname || "").trim();
    const parts = path.split(/[\\\\/]/).filter(Boolean);
    const candidate = parts.at(-1) || "";
    return /\\.html?$/i.test(candidate) ? sanitizeHtmlFileName(candidate) : "";
  }
  ...
  function startPrintFlow(statusMessage) {
    ...
    if (shouldBlockBrowserPrint(printJob)) {
      window.alert("当前打印范围较大，建议导出到 Excel 后打印");
  ...
  function createPrintJob() {
    ...
    return {
      ...
      sheetName: table.sheetName || "Sheet1",
    };
  }
